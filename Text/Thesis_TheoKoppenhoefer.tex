...
\todoin[]{
  ...
  \begin{itemize}
    ...
    \item update images
    \item add caption to Morse proof sketch, adjust size and positioning
  \end{itemize}
}
...
\td{The previous definition is not quite natural.}
...
\begin{definition}[Stagnation points]\label{df:nonDegeneracy}
  ...
  Boundary points which are non-regular essential stagnation points are called \emph{irregular stagnation points}.
...
The following characterisation of the irregular boundary somewhat clarifies the definition:
...
\begin{corollary}[Density of harmonic Morse functions]\label{co:density_boundaryGeneric}
  ...
  \begin{enumerate}
    ...
    \item A stagnation point $x_\e$ of $u^\e$ lies in $\Sigma^+\brk*{u}$ if it lies in $\Sigma^{\geq0}\brk*{u^\e}$ 
      and analogously it lies in $\Sigma^-\brk*{u}$ if it lies in $\Sigma^{\leq0}\brk*{u^\e}$. Additionally $\pm u^\e$ have no
...
\begin{proposition}[Negative answer to question \ref{qu:flowthroughStagnationPoint} in $d=2$ dimensions]\label{pr:n2_negativeResult}
  Let $X\subseteq\R^2$ be a simply connected planar compact manifold with corners and let $f\colon X\to\R$ be harmonic, non-degenerate on $\interior\brk*{X}$ and without irregular critical points.
...
\begin{proof}
  ...
  Then we can cut the domain along a smooth curve $\Gamma\subseteq\interior\brk*{X}$ such that the endpoints $\gamma=\partial\Gamma$
...
\begin{proposition}[Negative answer for cylinders, {\cite{Wahlen2023}}]\label{pr:n3_inflowOutflowCylinder}
  ...
  the lid $\brk[c]{0}\times U=\Sigma^+$ is the strictly entrant boundary and
  the lid $\brk[c]{1}\times U=\Sigma^-$ is the strictly emergent boundary. 
...
\begin{proof}
  ...
  by assumption and $\partial_1 f>0$ on the lids $\brk[c]*{0,1}\times U$ there exists a point
...
\begin{proposition}[Condition on the interior type numbers]\label{pr:n3_inflowOutflowRels}
  ...
  $\Sigma^+\subseteq\Sigma_{\geq0}$. Additionally we require that $\gamma\leftdef\partial\Sigma_{\leq0}$ is a one-dimensional manifold with corners homeomorphic to the
  circle $S^1$.
  Then we have that
  \begin{align*}
    M_1-M_2=0\,.
...
\begin{proposition}[Condition on the type numbers]
  Let $X\subseteq\R^3$ be a compact three-dimensional manifold with corners.
