...
In order to avoid the technical intricaties involved with manifolds with corners we assume that
...
\subsection{On assuming non-degeneracy}\label{se:assuming_nonDegeneracy}
...
\begin{corollary}[Density of boundary generic functions]\label{co:density_boundaryGeneric}
  ...
  Assume that $u$ has no critical points on $\Sigma^0$.
  Then there exists a $\delta>0$ such that for almost every $\e\in B_\delta\subseteq\R^d$ we have that
  \begin{enumerate}
    \item $u^\e$ given is non-degenerate on $X_j$ \label{co:nonDegeneracy_density_nonDeg}
  ...
  \end{enumerate}
  Here $u^\e$ is given by equation \eqref{}. In addition we have as $\e\to0$ that
  \begin{enumerate}
    \item $u^\e\to u$ uniformly
    \item The critical points of $u^\e$ converge uniformly to critical points of $u$
    \item The critical points of $u^\e$ and $u$ can be put into a one-to-one correspondence preserving index and the property
     of being emergent or entrant
  \end{enumerate}
\end{corollary}
\begin{proof}
  \td{Fill in the details for the following. Note the parallels to Morse's proof in \cite{Morse1970}.}
  We first show that we can choose a $\delta>0$ such that for all $\e\in B_\delta\subseteq\R^d$ we have no critical points on $\Sigma^0\brk*{u^\e}$.
  Assume not. Then there exist sequences $\e_k\to0$ and $x_k$ of critical points of $u^{\e_k}$ on $\Sigma^0(u^{\e_k})$.
  By compactness of $X$ we can assume that $x_k\to x$ for some $x\in X$ after taking a subsequence.
  After taking a further subsequence we can also assume that all $x_k$ lie in a stratum $X_j$.
  The condition that $x_j$ are critical points and lie in $\Sigma^0\brk*{u^{\e_j}}$
  means that there exists a stratum $X_{j-1}$ such that $x_j$ is also critical point of this stratum.
  But then $x$ is also a critical point of $X_{j-1}$ for $u$ since $u^\e\to u$.
  This implies that $x$ is a critical point of $X_{j-1}$, but $x\in\overline{X}_j$ and hence $x\in\Sigma^0$
  is a critical point. A contradiction.
  ...
  \td{Show a version of the second part of the corollary}
\end{proof}
...
\begin{proof}
  ...
  \end{align}
  By compactness of $\Sigma$ we can assume that $x_k\to x$ in $\Sigma$ after taking a subsequence. 
...
\begin{proposition}
  Let $d=2$ and $X$ be simply connected such that $\Sigma$ is a differentiable manifold and let $f\colon X\to\R$ be tangency regular.
  Assume further that $\Sigma^-$ and $\Sigma^+$ are nonempty and simply connected. Then $f$ has no interior critical point.
\end{proposition}
...
\begin{proof}
  ...
  Then we can cut the domain along a curve $\Gamma$ such that the endpoints
  ...
  \end{figure}
  In words, the vector field $u=\nabla f$ is essentially parallel to the boundary $\Sigma$ in a neighbourhood of $x_1$.
  We assume that we chose $\Gamma$ such that the acute angle between $u$ and $\Gamma$ at $x_1$ is on the side where $u$ flows into the new domain.
  ...
  \begin{align}
    \mu^+_j=\ind_{\Gamma,j}(f)+\delta_{j0}\ind_{\Sigma^0,j}(f)\label{eq:n2_disproof_morse_muplus}
  \end{align}
  ...
  \end{align}
  where we took into account that $\ind_{\Sigma^0,j}(-f)=0$.
  ...
  \begin{align*}
    M^-+M^+-\ind_{\Sigma^0,0}(f)=-2\chi(X)=-2\,.
  \end{align*}
  Since $\ind_{\Sigma^0,0}(f)\leq2$ and $M^\pm\geq0$ we must in fact have $M^\pm=0$ from which the claim follows.
...
\begin{proof}
  ...
  \begin{align}
    \mu^+_j=\ind_{j,\Gamma}(f)\,.\label{eq:n3_disproof_morse_muplus}
  \end{align}
  Analogously we have on $X^-$ that
  \begin{align}
    \nu^-_j=\ind_{j,\Gamma}(-f)\,.\label{eq:n3_disproof_morse_numinus}
  \end{align}
  ...
  \begin{equation}
    \begin{aligned}
      ...
      \ind_{2,\Gamma}(u)=\ind_{0,\Gamma}(-f)\label{eq:n3_disproof_morse_relationIndex} 
    \end{aligned}
  \end{equation}
  ...
  \begin{equation}
    \begin{aligned}
      \mu^+_0=\nu^-_2 \\
      \mu^+_1=\nu^-_1 \\
      \mu^+_2=\nu^-_0\label{eq:n3_disproof_morse_relationMuNu}
    \end{aligned}
  \end{equation}
  ...
  We now subtract equation \eqref{eq:n3_disproof_morse_morseineqplus} from \eqref{eq:n3_disproof_morse_morseineqminus}
