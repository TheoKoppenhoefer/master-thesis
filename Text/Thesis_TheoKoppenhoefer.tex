...
\tdrewrite{State this as the number of non-degenerate critical points is finite}
...
\begin{proposition}
  ...
  the lid $\brk[c]{1}\times U=\Sigma^-$ is the emergant boundary. 
...
\begin{example}[A harmonic vector field with simply connected entrant boundary]
  ...
  \begin{align}
    0&=\brk*{s^2-1}^2\brk*{\brk*{s^2+1}-r\brk*{s^2-1}}+x_1^2\brk*{3r\brk*{s^2-1}-\brk*{s^2+1}}+2x_1\brk*{s^2+1}x_2 \\
    &=\brk*{s^2+1}\brk*{\brk*{s^2-1}^2-x_1^2}-r\brk*{s^2-1}^3+3r\brk*{s^2-1}x_1^2+2x_1x_2\brk*{s^2+1}\\
    &=p_\e(x)
  \end{align}
  Our aim is to show that the real variety defined in this way is compact and has precisely one connectivity component.
  It then follows from the Jordan curve theorem that this variety splits the plane into two components, namely one where 
  $p_\e$ is positive and one where it is negative. The image of these sets under the stereographic projection is then precisely
  $\Sigma^+$ and $\Sigma^-$ respectively and it follows that these sets are simply connected.
  Dividing this equation by $r$ and considering the limit $r\to\infty$ we obtain the relation
  \begin{align*}
    0&=\brk*{s^2-1}\brk*{\brk*{s^2-1}^2-3x_1^2} \\
    &=p(x)
  \end{align*}
  which vanishes precisely if $x\in S^1$ or
  \begin{align*}
    0=\brk*{s^2-1}^2-3x_1^2
  \end{align*}
  substituting $\tix_k=x_k^2$ we can write this as
  \begin{align*}
    0&=\brk*{\tix_1+\tix_2-1}^2-3\tix_1\\
    &=\tix_1^2+2\tix_1\tix_2+\tix_2^2-5\tix_1-2\tix_2+1 \\
    &=\brk*{\tix_1+\tix_2}^2-5\tix_1-2\tix_2+1
  \end{align*}
  which is the equation of a conic section. Since we are assuming $\tix\in\R_{\geq0}^2$
  we obtain the section of a parabola $C$ in the upper right quadrant intersecting the $\tix_1$-axis at the points
  \begin{align*}
    \vect{a_\pm & 0}=\vect{\frac{5\pm2\sqrt{6}}{2} & 0}^\top
  \end{align*}
  and touching the $\tix_2$-axis at the point $e_2$. This is a connected curve.
  Now taking the square root we obtain four curves
  \begin{align*}
    C_{\circ_1\circ_2}=\brk[c]*{\vect{\circ_1\sqrt{\tix_1}&\circ_2\sqrt{\tix_2}}^\top\colon\tix\in C}
  \end{align*}
  for signs $\circ_1,\circ_2\in\brk[c]*{+,-}$. These curves intersect precisely at the points $\pm e_2$
  and at the four points on the $x_1$-axis given by
  \begin{align}
    \vect{\pm \sqrt{a_+}&0}^\top\qquad\vect{\pm \sqrt{a_-}&0}^\top\,.\label{eq:n3_example_rootsOfZeroes}
  \end{align}
  Since we have that
  \begin{align*}
  ...
  \begin{align*}
    p(x) =
  \end{align*}
  We also take the second derivative
  \begin{align*}
    D^2p(x)=6\begin{bmatrix}
  ...
  \begin{align*}
    q_\e(x)=\e\brk*{\brk*{s^2+1}\brk*{\brk*{s^2-1}^2-x_1^2}+2x_1x_2\brk*{s^2+1}}
  \end{align*}
  to our variety we 

  we also calculate for this part
  \begin{align*}
    \nabla q_\e(x)=2\e\vect{3x_1^2x_2 + x_1x_2^2 + x_2^3 - x_1 + x_2 \\ x_1^3 + x_1^2x_2 + 3x_1x_2^2 + 2x_2^3 + x_1}
  \end{align*}
  and
  \begin{align*}
    D^2q_\e(x)=2\e\begin{bmatrix}
  ...
  \begin{align*}
    D^2q_\e(\pm e_2)=2\e\begin{bmatrix}
      0 & 4 \\
      4 & 6
    \end{bmatrix}
  \end{align*}
