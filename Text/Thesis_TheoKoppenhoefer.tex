...
% \maketitle 
\includepdf[pages={1}]{../Mall/2024_Koppenhoefer_E16.pdf}
...
The second part revolves around harmonic vector fields
...
Som en del av examensarbetet formulerar vi en version av Morse-teorin enligt \cite{Braess1974} och \cite{Agrach1991}
...
%     \item rework figures
%     \item possibly add to literature: Braess, Mond and Ballesteros - Singularities of mappings
...
\begin{question}[Flowthrough with stagnation point, {\cite{Wahlen2023}}]\label{qu:flowthroughStagnationPoint}
...
\begin{question}[Harmonic vector fields without inflow or outflow, {\cite{Wahlen2023}}]
...
\nomenclature{$D$}{Derivative or differential.}
...
\nomenclature{$A^*$}{Dual cone of a set $A$. Defined analogous to equation \eqref{eq:emergentEntrantBd_dualCone}.}
...
In many proofs a flow is constructed along which $f$ increases.
...
\begin{definition}[Non-degeneracy and the Morse index]\label{df:nonDegeneracy}
...
\begin{example}[Stagnation points]\label{ex:n2_hvf_index}
...
Notice that it follows from the definition that a stagnation point of $u_j$ is irregular iff it
...
We are now in a position to define interior and boundary type numbers which is inspired by
...
\begin{lemma}[Interval without critical value, {\cite[Satz 4.1]{Braess1974}}]
  \label{le:Morse_homotopicNoCP}
...
\end{lemma}
\begin{proof}[Idea of proof]
  % This statement follows from \cite[Satz 4.1]{Braess1974} where we note that our notion of essential critical point of $f$ corresponds to the notion of a $\brk*{+}$-critical point of $-f$ in \cite{Braess1974}.
...
%   \end{align*}
%   since $C_{f(x)}W=\R$ for $f(x)<t$ and 
%   $$Df_t\big\vert_{x}\brk*{C_xX}=Df\big\vert_{x}\brk*{C_xX}\supseteq\R_{\leq0}$$
%   for $f(x)=t$
%   since $f$ has no essential critical values on the interval $[a,b]$.
%   Thus $f_t$ is transverse to $W$ for each $a\leq t\leq b$ and the result follows.
% \end{proof}
A similar result is given in \cite[Theorem 7]{Handron2002} and \cite{Agrach1991}.
...
\begin{proposition}[Morse's lemma, {\cite[Lemma 5]{Handron2002}}]\label{pr:Morse_lemma}
...
\begin{lemma}[Interval with critical value, {\cite[Theorem 8]{Handron2002}} and {\cite[Satz 7.1]{Braess1974}}]
...
The following result and proof comes from \cite[§6]{Douglas2020} and is a negative answer to Question \ref{qu:flowthroughStagnationPoint}
for the cylinder where one lid is the entrant and the other lid is the emergent boundary.
\begin{proposition}[Negative answer for cylinders, {\cite[§6]{Douglas2020}}]\label{pr:n3_inflowOutflowCylinder}
...
% Proposition \ref{pr:n3_inflowOutflowCylinder}
...
\begin{example}[A harmonic function with interior critical point and simply connected entrant and emergent boundaries]
...
From now onwards we assume that $p_1$ and $P_2$ are given by equations \eqref{eq:n3_polynomial_defp1} and \eqref{eq:n3_polynomial_defP2}.
...
\nomenclature{$d_H(A,B)$}{Hausdorff distance. See equation \eqref{eq:def_HausdorffMetric}.}
...
\begin{proposition}[Existence of a cutoff function]\label{pr:n3_thickening_cutoff}
  ...
  \end{enumerate}
\end{proposition}
\nomenclature{$\brk[a]*{h}_X$}{Expectation value $\int_Xh$ of a function $h\colon X\to Y$.}
...
% It then follows from the fact that $u$ is regular and from Proposition \ref{pr:characterisation_irregular_bd} that the stagnation points of $u$ are not stagnation points of higher strata. 
...
As before we consider harmonic vector fields
...
We can also give a second condition on the domain topology which is a version of the hairy
ball theorem.
\begin{proposition}[Condition on the boundary topology]
  ...
  Let $X$ be a orientable manifold with smooth boundary and $u\colon X\to TX$ be a smooth vector field without boundary stagnation points.
...
% \tikzset{external/export next=false}
% \tikz[remember picture,overlay, shift=(current page.center)] \node[opacity=1,inner sep=0pt] at (0,-5cm){\includegraphics[width=1.2\paperwidth,height=0.8\paperheight]{../Art/circular_001_colorised.pdf}};
...
\newpage
\tikzset{external/export next=false}
\tikz[remember picture,overlay] \node[opacity=0.7,inner sep=0pt] at (current page.center){\includegraphics[width=1.8\paperwidth,height=1.6\paperheight]{../Art/chaos_001_colorised.pdf}};
\thispagestyle{empty}
\newpage

\includepdf[pages={2}]{../Mall/2024_Koppenhoefer_E16.pdf}
