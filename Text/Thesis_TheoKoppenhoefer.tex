...
%   Let $U$ be an open bounded set such that $P_1\brk*{0,\cdot}$ is non-singular on an open neighbourhood of $\overline{U}$
%   Then there exists a $\delta>0$ such that for all $\e<\delta$
%   we have that the Hausdorff distance satisfies 
%   $d_H\brk*{V_U\brk*{P_1},V_U\brk*{P_1\brk*{0,\cdot}}}<\delta$ and 
%   $V_U\brk*{P_1}$ is homotopic to $V_U\brk*{P_1\brk*{0,\cdot}}$.
% \end{proposition}
% \begin{proof}
%   Since $P(0,\cdot)$ is non-singular on an open neighbourhood of $\overline{U}$ it follows that $DP(0,\cdot)$ is bijective on every point on
%   $\overline{U}$.
...
\begin{proof}
  % Then we calculate
  % \begin{align}
  %   V_U\brk*{P_1(\e,\cdot),p_2}\cong V_W\brk*{P_1\brk*{\e,x_1,x_2,\sqrt{r^2-x_1^2-x_2^2}}}
  % \end{align}
  % are diffeomorphic.
  % Rewriting
  % \begin{align*}
  %   0=-x_1^3+x_1x_2^2+x_1^2-x_2^2+x_2\sqrt{r^2-x_1^2-x_2^2} \\
  %   \Leftrightarrow x_1^3-x_1x_2^2-x_1^2+x_2^2=x_2\sqrt{r^2-x_1^2-x_2^2}\\
  %   \Leftrightarrow \brk*{x_1^3-x_1x_2^2-x_1^2+x_2^2}^2=
  % \end{align*}
