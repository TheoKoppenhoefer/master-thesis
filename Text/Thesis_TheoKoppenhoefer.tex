...
\todoin[]{
  ...
  \begin{itemize}
    ...
    \item Fix issue with examples
    \item Find modern version of complex analysis counterpart
    \item Vector identities for curl and div in R2
    \item Harmonic vector fields, find up to date reference \textrightarrow prove used identities
    \item Look up Leau-Fatou flower
    \item Look up Bocher theorem
    \item Give a complex analysis proof of the number of critical points without inflow or outflow \textrightarrow does this generalise to one inflow, outflow?
...
We call a boundary point $x$ \emph{ordinary} if it is not tangent, that is $u(x)\not\in T_x\Sigma$.
\begin{proposition}
...
\end{proposition}
\begin{proof}
  \ruggedtodo[inline]{insert proof here}
\end{proof}
...
\todoin[]{
\begin{itemize}
  \item betti numbers are finite
\end{itemize}
}

\subsection{The Morse inequalities}

\ruggedtodo[]{How do we know that the betti numbers are finite?}
We state the Morse inequalities.\ruggedtodo[]{More citations. Check if $C^2$ is really sufficient for Morse.}
...
\begin{corollary}[Morse inequalities for $f$ harmonic, $d=3$]
  ...
  \end{align*}
\end{corollary}
\ruggedtodo[]{Give a classical example of a Morse function to determine the betti numbers.}
...
\end{figure}\ruggedtodo[]{Update this graphic.}
...
$e_2$.\ruggedtodo[]{Show this.} One can further see from figure \ref{pl:n2:hvf:InflowOutflow:asymmetric}
