...
Analogously we can define the vector field
\begin{align*}
...
\begin{definition}[Emergent and entrant boundary]\label{df:emergentEntrantBd}
  ...
  Analogously we define the \emph{emergent} and \emph{strictly emergent boundary}.
...
\begin{corollary}[Density of boundary generic functions I]
...
\begin{proposition}
  ...
  Then $\partial\Sigma^+=\partial\Sigma^-$.
\end{proposition}
\begin{proof}
...
We summarise this section with the following lemma.
\begin{lemma}[Density of boundary generic functions]
  Let $u\colon X\to T^*X$ and $X$ be a stratified space. Then there exists a dense subset $E\subseteq B_r\R^d$
  for some $r>0$ such that
  \begin{enumerate}
    \item the $u^\e$ are boundary generic
    \item $u^\e\to u$ uniformly
    \item The critical points of $u^\e$ converge uniformly to the critical points of $u$
    ...
    \item The boundaries $\Sigma^+$ and $\Sigma^-$ are conserved
  \end{enumerate}
  as $\e\to0$ for $\e\in E$.
\end{lemma}
\begin{proof}
  ...
  We note that since $X$ is compact we have that $\nabla u_\e\to \nabla u$ 
...
\begin{proof}
  ...
  stratification $X^+\cup X^-$ as in section \ref{se:assuming_nonDegeneracy}.
