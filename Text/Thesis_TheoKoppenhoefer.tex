...
The overarching theme of this thesis can be summarised in two broad questions.
The first question is when it is possible to have a harmonic vector field $u$
on a bounded domain with interior stagnation points such that the boundary set
on which $u$ enters the domain is connected and the set on which $u$
leaves the domain is connected.
The answer in two dimensions is essentially `no'.
If one however allows for holes in the domain the answer becomes a `yes'
...
inflow and outflow sets.

The second question revolves around whether it is possible to have a harmonic vector field
with interior stagnation points such that the vector field at the boundary is tangential.
More generally we consider harmonic vector fields without stagnation points
on the boundary.
...
As part of the thesis we also formulate a version of Morse theory by \cite{Agrach1991}
...
\todoin[]{
  ...
  \begin{itemize}
    ...
    \item Find proper title.
    \item Mention \cite{Shelton1980} and \cite{Morse1970} in introduction
    \item Send Erik Latex commands
  \end{itemize}
  Some questions
  \begin{itemize}
...
Other examples is the heat flow in a system that has reached steady state or
...
correspond to critical points of a harmonic function.
The study of these points has a long tradition.
Walsh gives in \cite{Walsh1950} comprehensive overview of the state of knowledge about the position
and number of critical points of harmonic functions in 1950.
...
answer questions \ref{} and \ref{}.

% The thesis starts out with some mathematical preliminaries. After that we show the density of
% Morse functions. In the second chapter we state the Morse inequalities.
% In chapter 3 we 
\td{outline structure of thesis}
...
In general a harmonic vector field $u$ is not the gradient of a harmonic function like in for instance example \ref{ex:n2_flowthroughHole}.
...
\begin{question}[Flowthrough with stagnation point]\label{qu:flowthroughStagnationPoint}
  Does there exist a region $X\subseteq\R^d$ homeomorphic to a ball with flow $u$ through the region such that
  \begin{enumerate}
...
we have precisely $X_{j}\precsim X_{j+1}$ for $j\in\brk[c]*{1,\dots,d}$.
...
For completeness we also give the definition of the contingent cone:
...
\td{Add dual cone definition to cube. Explain relative interior.}
...
\begin{example}\label{ex:n2_hvf_canonical}
  ...
  one obtains that
  \begin{align*}
    \Sigma^{\leq0}&=\brk*{I\times\brk[c]*{1}}\cup \brk*{I\times\brk[c]*{-1}}\\
    \Sigma^{\geq0}&=\brk*{\brk[c]*{1}\times I}\cup \brk*{\brk[c]*{-1}\times I}\,.
  \end{align*}
...
\begin{remark}[Contingent cone]
  ...
  \begin{figure}
    ...
    \begin{subfigure}{0.45\textwidth}
      ...
      \caption{$\alpha<\pi/2$}
    \end{subfigure}
    \caption{Illustration of a planar contingent cone (yellow) and its dual cone (red).}
...
We have however that $x\in\partial X_{j+1}$. The concept of tangent space $T_xX_{j+1}$ and the projection $\pi_{j+1}$
can thus be extended to $\partial X_{j+1}$ by continuity and so we can reasonably speak of evaluating $u_{j+1}$ at the point $x$.
\begin{proof}
  ...
  and after taking a subsequence that $w_k'$ converge to a $w'\in C_xX$. But then we have by continuity that
...
We can now state a weakened version of the Thom's transversality theorem which is proven in \cite[§3 Theorem 2.7]{Hirsch1994}:
...
The main idea in the proof of the Morse inequalities is to inspect how the topology of the manifold $X^a$ changes as we vary $a$.
...
as to why equation \eqref{eq:MorseIneq_equality} is reasonable.
...
\begin{proposition}[Morse's lemma, {\cite[Lemma 5]{Handron2002}}]
  ...
  local chart $\varphi\colon H_{d-n}^d\supseteq V\to U$ such that
  \begin{align}
...
\begin{lemma}[Interval with critical value, {\cite[Theorem 8]{Handron2002}} and {\cite[Theorem 2.2]{Agrach1991}}]
  ...
  If the interval $[a,b]$ contains one critical value $a<c<b$ with corresponding non-degenerate essential critical point
  $x$ of index $k$
