...
In three dimensions we do not find such an example but the Morse index theorem implies
...
och sammanhängande inträdes- och sammanhängande utträdesrand. Svaret i två dimensioner är
...
polynom på bollen som har inre stagnationspunkter och enkelt sammanhängande inträdes- och
utträdesrand.
...
hittar vi inget sådant exempel, men Morse indexteorem implicerar att för ett sådant exempel
...
\tikz[remember picture,overlay] \node[opacity=0.3,inner sep=0pt] at (current page.center){\includegraphics[width=1.8\paperwidth,height=1.6\paperheight]{../Art/centering_003.pdf}};
...
\todoin[]{
  ...
  \begin{itemize}
    ...
    \item Perhaps give a classical example of a Morse function to determine the Betti numbers.
    \item Insert figure of that diffeomorphism.
    \item rework figures
    \item Look up Floer homology and Conley index theorem
    \item add literature, check that all literature is used.
    \item Think about the structuring of the stagnation point definitions
    \item acute angle to the vector field (p.27)
    \item Check that I get the assumption of irrotationality right
    \item Check that I get my differential geometry right: Do I need the manifold to be embedded, compact, etc?
  \end{itemize}
}
...
\begin{question}[Flowthrough with stagnation point]\label{qu:flowthroughStagnationPoint}
  ...
  \end{enumerate}
\end{question}
This question is in part inspired by the requirement that $u$ be free of stagnation points for an existance result of steady, inviscid
and incompressible flows with nonvanishing vorticity in $\R^3$
given in \cite{Alber1992}.
The answer will turn out to be `yes' for dimensions $d\geq3$ and essentially `no' for $d=2$ dimensions.
...
\begin{question}[Harmonic vector fields without inflow or outflow]
  Let $u\colon X\to\R^d$ be a harmonic vector field in a domain $X\subseteq\R^d$ such that at every boundary point it is tangential to the boundary
  and non-vanishing.
...
\begin{question}[Harmonic vector fields without boundary stagnation points]
  ...
  Let $u\colon X\to\R^d$ be a harmonic vector field on a domain $X\subseteq\R^d$ without boundary stagnation points.
  What can be said about the relation between the number of stagnation points and the domain topology?
\end{question}
...
In $d=3$ dimensions the Morse index theorem implies that the number of stagnation points has to be even and the Euler characteristic of the
...
The following chapter gives an essentially negative answer to question \ref{qu:flowthroughStagnationPoint} in the case of two dimensions.
...
Here we use the Morse index theorem to give an elegant relation
between the number of stagnation points and the domain topology for the planar case. 
We also give a condition on the number of interior stagnation points and the domain topology in three dimensions.
...
% on the number of interior stagnation points and the domain topology.
...
After this we define manifolds with corners, stratified spaces, tangent spaces and the entrant and emergent boundaries.
...
Unless otherwise stated we denote by $X\subset\R^d$ a compact subset of $\R^d$ with boundary $\Sigma=\partial X$ and nonempty interior
$\openX$.
...
More generally, we call a vector field $u\colon X\to TX$ \emph{irrotational} if it is locally the gradient of a function $f\colon X\to\R$.
...
\nomenclature{$u\colon X\to\R^d\text{ or }TX$}{A harmonic vector field.}
\nomenclature{$\Delta$}{Big capital delta. Here used for the Laplace operator. In cartesian coordinates $\Delta=\sum_j\partial_j^2$.}
\nomenclature{$\nabla$}{Gradient.}
\nomenclature{$\nabla^\perp$}{Orthogonal gradient. Given by equation \eqref{eq:n2_nablaPerp}.}
\nomenclature{$\curl$}{Curl operator. Given in $\R^2$ by equation \eqref{eq:n2_curl}.}
\nomenclature{$\diver$}{Divergence operator. In cartesian coordinates $\diver=\sum_j\partial_j$.}
...
\end{definition}
% Note that in the definition of diffeomorphism we used the following notion of differentiability:
% Let $X$ and $Y$ be differentiable manifolds without boundary.
% We call a function $F\colon A\to B$ from a subset $A\subseteq X$  to a subset $B\subseteq Y$ differentiable if
% there exists around every $x\in A$ an open subset $U_x\subset Y$ differentiable extension of $F$ to an open subset of $A$
...
\nomenclature{$\simeq$}{$x\simeq y$ means that $x$ is homotopic to $y$.}
...
\begin{definition}[Tangent space, {\cite[§1.2]{Jost2002}}]
...
\end{definition}
In general we will also require $X$ to be a Riemannian manifold in order to be able to talk about
gradients.
...
\end{example}
\nomenclature{$\relint$}{Relative interior as in definition \ref{df:relativeInterior}.}


We remind the reader of the concept of relative interior.
\begin{definition}[Relative interior, {\cite[Chapter 2.1.3]{Boyd2004}}]\label{df:relativeInterior}
...
\begin{definition}[Emergent and entrant boundary]\label{df:emergentEntrantBd}
  ...
  Here the set
  \begin{align}
    \brk*{C_xX}^*=\brk[c]*{w\in T_xX\colon w\cdot w'\geq0\text{ for all }w'\in C_xX}\eqref{eq:emergentEntrantBd_dualCone}
  \end{align}
  ...
  \end{align*}
\end{definition}
\nomenclature{$T_xX$}{Tangent space of $X$ at $x$. See definition \ref{df:tangentSpace}.}
\nomenclature{$C_xX$}{Contingent cone of $X$ at $x$. See definition \ref{df:contingentCone}.}
% \nomenclature{$T_x^*X$}{Cotangent space of $X$ at $x$. See for instance \cite[Chapter 6]{Hirsch1994} or \cite[§1]{Jost2002}.}
\nomenclature{$A^*$}{Dual cone of $A$. Defined analogous to equation \eqref{eq:emergentEntrantBd_dualCone}.}
...
This test particle
will then move away from the point $x$ along the boundary $\Sigma^+$.
...
\begin{remark}[The contingent cone and its dual]
...
Given a vector field $u\colon X\to TX$ and a stratification $X_j$ of $X$ we can construct for every
$j\in\cJ$ a vector field
\begin{align*}
  u_j\colon X_j\to TX_j\,.
\end{align*}
% Here $TX_j$ denotes the cotangent space of the manifold $X_j$ which is defined for instance in \cite[Chapter 6]{Hirsch1994}.
...
\nomenclature{$u_j$}{Projection of $u$ to the tangent bundle $TX_j$. See equation \eqref{eq:def_projectionUStratification}.}
\nomenclature{$\pi_j$}{Orthogonal projection onto the tangent bundle $TX_j$. See equation \eqref{eq:def_projectionStratification}.}
\begin{definition}[Stagnation points]\label{df:stagnationPoints}
...
\begin{definition}[Non-degeneracy]\label{df:nonDegeneracy}
  ...
  that is $x$ lies in the strictly entrant boundary $\Sigma^-$ or is a non-degenerate interior stagnation point.
  Essential boundary stagnation points of $u$ and $-u$ which are not regular are called \emph{irregular stagnation points}.
  % We call the set of all irregular stagnation points the irregular boundary $\Sigma^\text{irr}$.
  $u_j$ is called \emph{regular} if it is essentially non-degenerate and has no irregular stagnation points.
  We can define the \emph{$k$-th type number} $\ind^\text{M}_{j,k}(u)$ of the
  stratum $X_j$ to be the number of regular stagnation points of $u_j$ of Morse index $k$,
  that is
  \begin{align}
    \ind^\text{M}_{j,k}(u)\leftdef\#\brk[c]*{x\in\critical_j(u)\colon x\text{ has index }k}\,.\label{eq:def_typenbr_strata}
  \end{align}
\end{definition}
\nomenclature{$\critical_j$}{Set of essential stagnation points. See definition \ref{df:stagnationPoints}.}
\nomenclature{$\ind^\text{M}_{j,k}$}{$k$-th type number on the stratum $X_j$. See equation \eqref{eq:def_typenbr_strata}.}
...
\begin{example}\label{ex:n2_hvf_index}
  ...
  of $u_1$ with index $0$.
  Now since $u\brk*{e_2}\in\relint\brk*{C_xX}^*$ we have that $e_2$ is in fact an essential and regular 
  stagnation point.
...
Notice that it follows from the definition that a stagnation point of $u_j$ is irregular if it is degenerate or
...
\begin{proposition}[Characterisation of irregular points]\label{pr:characterisation_irregular_bd}
...
\begin{definition}[Morse functions, {\cite[Definition 10.3]{Morse1969}}]
  ...
  \end{align}
\end{definition}
\nomenclature{$\ind^\text{M}_k$}{$k$-th type number. See equation \eqref{eq:def_typenbr_total}.}
\nomenclature{$\ind^\text{PH}_{j,x}$}{Poincaré-Hopf index of $x$ on the stratum $X_j$. See definition \ref{df:poincareHopf_index}.}
\nomenclature{$\ind^\text{PH}_j$}{Total Poincaré-Hopf index on the stratum $X_j$. See definition \ref{df:poincareHopf_index}.}
...
\begin{corollary}[Density of harmonic Morse functions]\label{co:density_boundaryGeneric}
  ...
  \begin{enumerate}
    ...
    %   a stagnation point $x^\e$ of $u^\e$ lies in $\Sigma^\pm\brk*{u^\e}$ iff it lies in $\Sigma^\pm\brk*{u}$.
      \label{co:nonDegeneracy_density_entrEmerg}
    % \item $u^\e$ and $-u^\e$ have no irregular stagnation points\label{co:nonDegeneracy_density_regularity}
...
\end{corollary}
Here we defined the \emph{$\eta$-neighbourhood} or \emph{tubular neighbourhood}
...
\begin{proof}
  ...
  % Let $C$ denote the set of essential stagnation points of $u_j$ on the stratum $X_j$.
  % Since $u_j$ is continuous and $U_x\leftdef\interior\brk*{C_xX}^*\oplus T_xX_j\subseteq T_xX$ is
  % open we also have that the preimage $u_j^{-1}\brk*{U}$ is open where we set $U=\bigcup_xU_x$.
  % As $u$ has no irregular stagnation points it follows that
  ...
  \begin{align}
    \Image\brk*{DF^\text{ev}_{(\e,x)}}=T_xTX_j\label{eq:nonDegeneracy:surjectiveDF}
  \end{align}
  since
  \begin{align*}
...
\end{example}
%
\begin{example}[In spaceland, {\cite[Table 1]{Cantarella2002}}]
...
%   critical point of Morse index $k$. Then there exists a $C^r$ chart $(\varphi,U)$ at $x$
%   such that we have
%   \begin{align*}
...
% Now $H_2\brk*{X,A}$ is generated by $2$-cycles which are identified if they, possibly augmented by portions of
% $A$, are the boundary of a $3$-chain (oriented volume).
% Here $2$-cycles denotes closed oriented surfaces and surfaces with boundary contained in $A$.
% A $2$-cycle is considered trivial if they, possibly augmented by $2$-chains (oriented surfaces) in $A$ are the boundary
% of a $3$-chain.
...
%  (theorem \ref{th:Morse_MorseIneq})
...
% We can now formulate the Poincaré-Hopf index theorem:
% \begin{theorem}[Poincaré-Hopf index theorem, {\cite[§6]{Milnor1965}}]\label{th:poincareHopf}
%   Let $u\colon X\to TX$ be a vector field on a compact manifold with corners $X\subset\R^d$ such that $\Sigma=\Sigma^+$. Then we have
...
% This theorem relates to the Morse inequalities in the following way:
% Let $u\colon X\to TX$ be a gradient field on a manifold with corners $X\subset\R^d$ such that $\Sigma=\Sigma^+$.
% Then it follows that $u=\nabla f$ has no essential boundary stagnation points and the equality of the Morse inequalities reads as
% \begin{align*}
%   \ind^{\text{PH}}(u)=\sum_k(-1)^kM_k=\sum_k(-1)^k\ind^\text{M}_k(f)=\chi(X)
% \end{align*}
% where we used relation \eqref{eq:poincareHopf_typeNbrs}.
% The Poincaré-Hopf index theorem does not require $u$ to be a gradient field however.
% We now show how corollary \ref{co:n2_noInflowNoOutflow} follows from the Poincaré-Hopf index theorem.
% From equation \eqref{eq:poincareHopf_typeNbrs} it follows for a Morse harmonic vector field $u$ as in corollary \ref{co:n2_noInflowNoOutflow} that $M=-\ind_\text{PH}(u)$.
% Since $u$ is tangential at the boundary we can modify $u$ close to the boundary
% without adding critical points
% in such a way that it points outward from $X$ on the boundary.
% Hence $\Sigma=\Sigma^+$.
% Corollary \ref{co:n2_noInflowNoOutflow} thus
...
\begin{proof}
  ...
  \end{align}
  % where we took into account that $\ind^\text{M}_{\gamma,k}(-f)=0$.
  In addition we have on the cut $\Gamma$ that the emergent critical points of $f$ on $X^+$ are the
  ...
  that $u$ has no irregular stagnation points on $\Gamma$. It also follows from the acute angle that $\Gamma$ forms with $u$ on $\gamma$ that
  ...
  % \begin{align*}
  %   \ind^\text{M}_{\gamma^+,k}\brk*{f^\e}=\ind^\text{M}_{\gamma^+,k}(f)\qquad\text{ and }\qquad 
  %   \ind^\text{M}_{\gamma^-,k}\brk*{-f^\e}=\ind^\text{M}_{\gamma^-,k}(-f)\,.
  ...
  % \begin{align}
  %   \ind^\text{M}_{j,X_k}\brk*{f^\e}=\ind^\text{M}_{j,X_k}(f)\qquad\text{ and }\qquad 
  %   \ind^\text{M}_{j,X_k}\brk*{-f^\e}=\ind^\text{M}_{j,X_k}(-f)
...
% \begin{conjecture}
%   Let $X\subset\R^2$ be a manifold with corners with Betti numbers $b_0=1$ and $b_1$.
...
\begin{proposition}[Condition on the interior type numbers]\label{pr:n3_inflowOutflowRels}
  Let $X\subset\R^3$ be a compact three-dimensional manifold with corners and
  let $f\colon X\to\R$ be a Morse harmonic function without irregular stagnation points.
  Let $\Sigma=\Sigma_{\leq0}\sqcup\Sigma_{\geq0}$ be a disjoint decomposition of the boundary into simply connected nonempty sets such that
...
\begin{proof}
  ...
  \end{align}
  % \begin{align}
  %   \ind^\text{M}_{\gamma^+,0}(f)+\ind^\text{M}_{\gamma^-,1}(-f)=2l
  % \end{align}
  % for some $l\in\N$.\ruggedtodo{elaborate this}
  We now turn our attention to $X^+$. Since no essential critical points lie on $\Sigma_{\geq0}$ 
  it follows for the boundary type numbers that
  \begin{align}
  ...
  entrant critical points on $\Gamma=\Gamma^-$ of $-f$ on $X^-$ and vice versa, that is
  \begin{equation}
    \begin{aligned}
  ...
  \begin{align*}
    0&=M_1^--M_2^-+M_1^+-M_2^++\ind^\text{M}_{\gamma^+,0}(f)+\ind^\text{M}_{\gamma^-,1}(-f)-\ind^\text{M}_{\gamma^+,1}(f)-\ind^\text{M}_{\gamma^-,0}(-f) \\
    &= M_1-M_2
  \end{align*}
  from which the claim follows.
  ...
  Thus $u$ has no irregular stagnation points on $X^+$ or $X^-$.
  Let $E^+,E^-\subseteq B_\delta$ be as in corollary \ref{co:density_boundaryGeneric} applied separately to the domains $X^+$ and $X^-$.
  Since $E^\pm$ are residual in $B_\delta$ we can in particular pick a $\e\in E^+\cap E^-$ by the Baire category theorem.
  % Since the critical points of $f$ lying on $\gamma$ are non-degenerate
...
\begin{theorem}[Positive answer to question \ref{qu:flowthroughStagnationPoint} in $\R^3$]\label{th:n3_polynomial_connectedInflowOutflow}
...
\nomenclature{$p_1$}{Polynomial given by equation \eqref{eq:n3_polynomial_defp1}.}
\nomenclature{$p_2$}{Polynomial given by equation \eqref{eq:n3_polynomial_defp2}.}
\nomenclature{$P_1$}{Homogenisation of $p_1$ given by equation \eqref{eq:n3_polynomial_defP1}.}
\nomenclature{$P_2$}{Homogenisation of $p_2$ given by equation \eqref{eq:n3_polynomial_defP2}.}
...
\begin{proposition}[Convergence of $\cV_\e$ at smooth points]\label{pr:n3_polynomial_convergenceSmoothSet}
  Let $U\subseteq \R^3$ be an open bounded set such that $\cV_0$ is smooth
...
\begin{proof}
  ...
  \end{alignat*}
  to be subsets of $\R^2$. Also note that $\tialpha^\bullet\leftdef\psi^{-1}\brk*{\alpha^\bullet}$.
  We require the following proposition which will be proved later:
  \begin{proposition}[Construction of $\tiC$ and $\tigamma$]\label{pr:n3_polynomial_regularH}
  ...
  We can pick $\eta$ smaller than the minimum distance between two arcs of $\widetilde{\cV_0}$ on $\tiU_{\rho}$.
  ...
  Since $\tigamma$ by proposition \ref{pr:n3_polynomial_regularH} splits $\tiW_{2\rho}$ and lies in $\tiC\cap\widetilde{\cV}_\e$ we must have that
...
It now only remains to show proposition \ref{pr:n3_polynomial_regularH}.
...
%   Let $X\subset\R^2$ be compact and $\gamma=V_X(p)\subseteq X$ a smooth one-dimensional variety
...
In the previous example we showed that $\Sigma^+$ is separated from $\Sigma^-$ by the variety
...
\nomenclature{$L^p\brk*{X;Y}$}{Lebesgue space of functions $u\colon X\to Y$.}
% \nomenclature{$\norm{\cdot}_{L^p\brk*{X;Y}}$}{Norm on $L^p\brk*{X;Y}$.}
\nomenclature{$W^{k,p}\brk*{X;Y}$}{Sobolev space of order $k$ for functions $u\colon X\to Y$.}
% \nomenclature{$\norm*{\cdot}_{W^{k,p}\brk*{X;Y}}$}{Norm on $W^{k,p}\brk*{X;Y}$.}
...
In the final chapter of this thesis we will discuss harmonic vector fields without
inflow or outflow through the boundary.
First we will state the Morse index theorem which generalises the Poincaré-Hopf index theorem.
...
% We will then discuss briefly how this result relates to the Poincaré-Hopf index theorem.
...
\begin{definition}[Poincaré-Hopf index, {\cite[Definition 1.1.1]{Brasselet2009}}]
...
\begin{lemma}[Relation to the Morse index, {\cite[§6, Lemma 4]{Milnor1965}}]
...
\begin{theorem}[Morse index theorem, {\cite[Theorem $A_0$]{Morse1929}}]
  Let $X$ be a compact orientable manifold with smooth boundary and $u\colon X\to TX$ a continuous vector field.
  Assume that all essential stagnation points of $u$ are isolated and not stagnation points of higher strata.
  Then we have that
  \begin{align*}
...
It then follows from the fact that $u$ is regular and from proposition \ref{pr:characterisation_irregular_bd} that the stagnation points of $u$
are not stagnation points of higher strata. Since all stagnation points are non-degenerate they are also isolated and
...
\begin{example}[Vector field with rotation]\label{ex:n2_vf_circular}
...
\begin{proposition}[Condition on the number of stagnation points]\label{pr:n2_hvf_noInflowNoOutflow}
...
\begin{proof}
  ...
  % and in the case that $u$ is non-degenerate on $\interior\brk*{X}$ we
  % have for the interior type numbers
  % \begin{align*}
  %   \\
  %   -M &\annotateAbove{\text{harmonicity}}{=}{0.1}{0} M_2-M_1+M_0 \\ \\
  %   &\annotateAbove{\text{equation \eqref{eq:poincareHopf_totalIndex}}}{=}{0.1}{0} \sum_j\ind^\text{PH}_j(u) \\ \\
  %   &\annotateAbove{\text{Morse index theorem}}{=}{0.1}{0} \chi\brk*{X}
  % \end{align*}
  % from which the claim follows.
\end{proof}
% \begin{proposition}[Condition on the number of stagnation points]\label{pr:n2_hvf_noInflowNoOutflow}
...
% \end{proposition}
% \begin{proof}
%   We slit the domain $X$ with a smooth curve $\Gamma$  such that it is homeomorphic to the disk as is 
%   depicted in figure \ref{fi:n2_hvf_slitDomain}.
%   Denote the slit domain by $\tiX$.
%   Since the number of interior stagnation points is 
...
%   We denote the boundary of $\Gamma$ by $\gamma=\partial\Gamma=\Gamma\cap\Sigma$
%   and define points $\brk[c]*{x_1,\dots,x_{2b_1}}=\gamma$.
%   Note that there are $2b_1$ many such points where $b_1$ is the first Betti number.
...
%   \begin{align}
%     \ind^\text{M}_{\gamma,0}(u)+\ind^\text{M}_{\gamma,0}(-u)=2b_1\,.\label{eq:n2_hvf_noInflowNoOutflow_relgamma}
%   \end{align}
%   Since there are no boundary stagnation points on $\Sigma$ the boundary type numbers satisfy
%   \begin{align}
%     \mu_k=\ind^\text{M}_{\Gamma,k}(u)+\ind^\text{M}_{\gamma,k}(u) \qquad\text{ and }\qquad
%     \nu_k=\ind^\text{M}_{\Gamma,k}(-u)+\ind^\text{M}_{\gamma,k}(-u)\label{eq:n2_hvf_noInflowNoOutflow_relationsMuIndex}
%   \end{align}
%   for all $k\in\brk[c]*{0,1}$.
%   All entrant stagnation points of $u$ on $\Gamma$ are also emergent
%   stagnation points of $-u$ on $\Gamma$ and vice versa and thus we have the relations
%   \begin{align}
%     \ind^\text{M}_{\Gamma,0}(u)=\ind^\text{M}_{\Gamma,1}(-u) \qquad\text{ and }\qquad
%     \ind^\text{M}_{\Gamma,1}(u)=\ind^\text{M}_{\Gamma,0}(-u)\,.\label{eq:n2_hvf_noInflowNoOutflow_relationsIndex}
%   \end{align}
%   Equations \eqref{eq:n2_hvf_noInflowNoOutflow_relationsMuIndex} and \eqref{eq:n2_hvf_noInflowNoOutflow_relationsIndex}
%   yield 
%   \begin{align}
%     \mu_0-\ind^\text{M}_{\gamma,0}(u)=\nu_1 \qquad\text{ and }\qquad
%     \mu_1=\nu_0-\ind^\text{M}_{\gamma,0}(-u)\,.\label{eq:n2_hvf_noInflowNoOutflow_relationsMuNu}
%   \end{align}
...
%   \begin{align*}
%     -2=2M-\ind^\text{M}_{\gamma,0}(u)-\ind^\text{M}_{\gamma,0}(u)
%     =2M-2b_1
%   \end{align*}
%   from which the claim follows.

%   The claim remains to be shown in the case that $u$ is not strongly Morse on $\tiX$.
%   Note that since $u$ has no boundary stagnation points on $\Sigma$ it also has no irregular stagnation points on $\Sigma$.
%   Since $\Gamma$ contains no interior stagnation points it also contains no irregular stagnation points by proposition
%   \ref{pr:characterisation_irregular_bd}. And due to the slated angle at which $\Gamma$ approaches the points $x_1,\dots,x_{2b_1}\in\gamma$
%   these points are also not irregular stagnation points. Now let
...
%   % we obtain that
%   % \begin{align}
%   %   \ind^\text{M}_{\gamma,k}\brk*{u^\e}=\ind^\text{M}_{\gamma,k}(u)\qquad\text{ and }\qquad 
%   %   \ind^\text{M}_{\gamma,k}\brk*{-u^\e}=\ind^\text{M}_{\gamma,k}(-u)
%   %   \label{eq:n2_disproof_morse_indexApprox}
%   % \end{align}
%   By the same corollary $u^\e$ has no stagnation points on $\Sigma$.
%   The claim then follows by the calculations above where we replace
%   $u$ with $u^\e$ and then note that $M^\e=M$ as $f$ is non-degenerate on $\interior\brk*{X}$.
% \end{proof}
% Note that the only place in the proof where we use that $u$ is harmonic is in the application of proposition \ref{pr:n2:hvf:simplyConnected}
% to show that $u$ is a gradient field on a simply connected domain and in the Morse inequalities in equations \eqref{eq:n2_hvf_MorseInequalities_1} and
% \eqref{eq:n2_hvf_MorseInequalities_2}. If we drop the assumption that $u$ be harmonic the Morse inequality used becomes
% \begin{align*}
%   M_2-M_1-\mu_1+M_0+\mu_0=-\chi\brk*{\tiX}\,.
% \end{align*}
% The same proof then leads in this case to the relation
% \begin{align*}
%   M_2-M_1+M_0=-\chi\brk*{X}
% \end{align*}
% between the interior type numbers and the domain topology for an irrotational Morse vector field without boundary
% stagnation points.
We now give an alternative and proof of proposition \ref{pr:n2_hvf_noInflowNoOutflow} using the argument principle.
...
\begin{corollary}[No in- or outflow]\label{co:n2_noInflowNoOutflow}
  ...
  strongly Morse harmonic vector field without inflow or outflow on
  $\Sigma$. Then we have the relation $M=-\chi(X)$ where $M$ is the number of stagnation points counting multiplicities
...
\begin{example}[No in- or outflow, \cite{Wahlen2023}]
...
The next example highlights the importance of the assumption that $u$ be strongly Morse in corollary \ref{co:n2_noInflowNoOutflow}.
\begin{example}[Stagnation points on the boundary, \cite{Wahlen2023}]\label{ex:n2_noInflowOutflow_irregular}
...
% \end{proof}
% We can also give a nontrivial condition on the interior type numbers of such a $u$ and the domain topology
% using the techniques of previous proofs:
% \begin{proposition}[Condition on the type numbers and domain I]\label{pr:n3_noInflowNoOutflow}
%   Let $X\subset\R^3$ be a compact three-dimensional manifold with boundary.
%   Let $u\colon X\to\R$ be a Morse harmonic vector field without boundary stagnation points.
%   Then the relation $M_1=M_2$ holds for the interior type numbers and the Euler characteristic of the domain
%   $\chi\brk*{X}=0$ vanishes.
% \end{proposition}
\begin{proposition}[Condition on the domain topology]\label{pr:n3_domainCond}
...
\begin{proof}
  ...
  \end{align*}
  and thus the Morse index theorem to the vector field $-u$ yields
  \begin{align}
...
\begin{corollary}[Condition on the type numbers and domain]\label{co:n3_conditionTypeNbrII}
  Let $X\subseteq\R^3$ be a compact three-dimensional manifold with smooth boundary and let further
  $u\colon X\to TX$ be a Morse harmonic vector field and no
...
% \end{proposition}
% \begin{proof}
%   Note that since we have no boundary stagnation points the boundary type numbers $\mu_k=0$ vanish 
%   and the Morse inequalities then imply that
%   \begin{align*}
%     M_2-M_1=-\chi\brk*{X}\,.
%   \end{align*}
%   Hence it is sufficient to show that $M_1=M_2$.
%   %
...
%   that it does not contain any interior stagnation points. We also denote the arcs at which $\Gamma$ meets $\Sigma$ by 
%   $\gamma_1,\dots,\gamma_{b_1}\subseteq\partial\Gamma$. Note that there are $b_1$ many such curves where $b_1$ denotes the first Betti number.
%   We can assume that the slit $\Gamma$ and the arcs $\gamma_j$ are manifolds with corners 
%   without irregular stagnation points. Using proposition \ref{pr:characterisation_irregular_bd} one can achieve this by choosing choosing $\gamma_j$ in a suitable manner
%   and letting $\Gamma$ approach
%   each $\gamma_j$ such that $\Gamma$ is not orthogonal to $u$ at points on $\gamma$.
...
%   $\Gamma^i$ and $\gamma_j^i$ with $i\in\brk[c]*{1,2}$.
...
%   Now we have that each $\gamma_j$ is homeomorphic to the circle $S^1$.
%   Since $f$ is non-degenerate the number of maxima and minima of $f$ on $\gamma_j^1\cup\gamma_j^2$ must be equal and thus
%   \begin{align}
%     \sum_i\brk*{\ind^\text{M}_{\gamma_j^i,0}(f)+\ind^\text{M}_{\gamma_j^i,1}(-f)}=\sum_i\brk*{\ind^\text{M}_{\gamma_j^i,1}(f)+\ind^\text{M}_{\gamma_j^i,0}(-f)}\,.
...
%   \begin{equation}
%     \begin{aligned}
%     \ind^\text{M}_{\Gamma^1,0}(\pm u)&=\ind^\text{M}_{\Gamma^2,2}(\mp u)\,, \\
%     \ind^\text{M}_{\Gamma^1,1}(\pm u)&=\ind^\text{M}_{\Gamma^2,1}(\mp u)\text{ and} \\
%     \ind^\text{M}_{\Gamma^1,2}(\pm u)&=\ind^\text{M}_{\Gamma^2,0}(\mp u)\,.
%     \end{aligned}\label{eq:n3_hvf_noInflowNoOutflow_relationsIndex}
%   \end{equation}
%   % and similarly we obtain on $\gamma_j^i$ the relations
%   % \begin{equation}
%   %   \begin{aligned}
%   %   \ind^\text{M}_{\gamma^1,0}(\pm u)&=\ind^\text{M}_{\gamma^2,1}(\mp u) \\
%   %   \ind^\text{M}_{\gamma^1,1}(\pm u)&=\ind^\text{M}_{\gamma^2,0}(\mp u)\,.
...
%   \begin{equation}
%     \begin{aligned}
%     \mu_k&=\sum_i\brk*{\ind^\text{M}_{\Gamma^i,k}+\sum_j\ind^\text{M}_{\gamma_j^i,k}}(f)\quad\text{and} \\
%     \nu_k&=\sum_i\brk*{\ind^\text{M}_{\Gamma^i,k}+\sum_j\ind^\text{M}_{\gamma_j^i,k}}(-f)\,.
...
%   \begin{equation}
%     \begin{aligned}
%     \mu_0-\nu_2&=\sum_{i,j}\ind^\text{M}_{\gamma_j^i,0}(f)\,, \\
%     \mu_1-\nu_1&=\sum_{i,j}\brk*{\ind^\text{M}_{\gamma_j^i,1}(f)-\ind^\text{M}_{\gamma_j^i,1}(-f)}\text{ and} \\
%     \mu_2-\nu_0&=-\sum_{i,j}\ind^\text{M}_{\gamma_j^i,0}(-f)\,.
%     \end{aligned}
%     \label{eq:n3_hvf_noInflowNoOutflow_relationsMuNu}
%   \end{equation}
%   For $f$ we have the Morse inequality
%   \begin{align}
%     M_2+\mu_2-M_1-\mu_1+\mu_0&=-\chi\brk*{\tiX} \label{eq:n3_hvf_MorseInequalities_1}
%   \end{align}
%   and for $-f$ the Morse inequality
%   \begin{align}
...
%   \begin{align*}
%     0&=2\brk*{M_2-M_1}
%     +\sum_{i,j}\brk*{\ind^\text{M}_{\gamma_j^i,0}(f)
%     -\ind^\text{M}_{\gamma_j^i,1}(f)
%     +\ind^\text{M}_{\gamma_j^i,1}(-f)
%     -\ind^\text{M}_{\gamma_j^i,0}(-f)} \\
%     &=2\brk*{M_2-M_1}
%   \end{align*}
%   from which the claim follows.

%   The claim remains to be shown in the case that $f$ is not strongly Morse on $X^+$ and $X^-$. In this case
%   observe that $f$ has no irregular stagnation points on $\Sigma$.
%   By the choice of $\Gamma$ and the $\gamma_j$ $f$ also contains no irregular stagnation points on these sets.
%   % As $\Gamma$ contains no interior stagnation points
%   % it follows from proposition \ref{pr:characterisation_irregular_bd} that $\Gamma$ also contains no irregular stagnation points.
%   % Due to the slanted angle at which $\Gamma$ approaches $\gamma$ and it follows that there also are no irregular stagnation points on
%   % $\gamma$.
%   Hence $u$ has no irregular stagnation points on $\tiX$.
%   Let $f^\e$ for $\e\in E$ be a strongly Morse function as in corollary \ref{co:density_boundaryGeneric}.
%   % Since $x_1$, $x_2$ are non-degenerate critical points of $f$
%   % due to the slanted angle at which
%   % $\Gamma$ approaches each $\gamma_j$
%   % we obtain that
%   % \begin{align}
%   %   \ind^\text{M}_{\gamma_j,k}\brk*{f^\e}=\ind^\text{M}_{\gamma_j,k}(f)\qquad\text{ and }\qquad 
%   %   \ind^\text{M}_{\gamma_j,k}\brk*{-f^\e}=\ind^\text{M}_{\gamma_j,k}(-f)
...
%   $\Sigma$.
%   The claim then follows by the calculations above where we replace
%   $f$ with $f^\e$ and then note that $M_1^\e=M_1$ and $M_2^\e=M_2$ as $u$ is non-degenerate on the interior $\interior\brk*{X}$.
% \end{proof}

% Note that the only place in the proof where we used the assumption that $u$ is harmonic is in the application of
% proposition \ref{pr:n2:hvf:simplyConnected} to show that $u$ a gradient vector field on simply connected domains and in the Morse inequalities in
% equations \eqref{eq:n3_hvf_MorseInequalities_1} and \eqref{eq:n3_hvf_MorseInequalities_2}. If $u$ is not necessarily harmonic the
% Morse inequality instead reads
% \begin{align*}
%   M_3-M_2-\mu_2+M_1+\mu_1-M_0-\mu_0=-\chi\brk*{\tiX}\,.
% \end{align*}
% We then obtain with the same proof the relation
% \begin{align*}
%   0=M_3-M_2+M_1-M_0\label{eq:n3_MorseIneq_general}
% \end{align*}
% between the type numbers for an irrotational Morse vector field without boundary stagnation points. By the same argumentation the Euler characteristic of the domain
% $\chi\brk*{X}=0$ also vanishes in this case.
% As a consequence of proposition \ref{pr:n3_noInflowNoOutflow} we obtain the following:
% \begin{corollary}[Condition on the type numbers and domain II]\label{co:n3_conditionTypeNbrII}
%   Let $X$ be a compact three-dimensional manifold with corners and $u\colon X\to\R^d$ a strongly Morse harmonic vector field
%   without inflow or outflow on the boundary $\Sigma$. The we have for the
%   relation $M_1=M_2$ between the interior type numbers and the Euler characteristic of the domain 
%   $\chi\brk*{X}=0$ vanishes.
% \end{corollary}

% With the aide of the Poincaré-Hopf index theorem we can use this to make a statement on the domain topology.
% \begin{corollary}[Condition on the domain topology I]
%   Let $X$ and $u$ be as in corollary \ref{co:n3_conditionTypeNbrII}. Then the Euler characteristic of the domain $\chi\brk*{X}=0$
%   must vanish.
% \end{corollary}
% \begin{proof}
%   As noted in \cite[§6, Lemma 4]{Milnor1965} we have the that the Poincaré-Hopf index at a stagnation point $x$ equals $\brk*{-1}^k$ where $k$ is the
%   Morse index of $x$.
%   It then follows from $M_1=M_2$ that the Poincaré-Hopf index $\ind_\text{PH}\brk*{u}=0$ vanishes.
%   We now modify the vector field in a neighbourhood of the boundary $\Sigma$ such that it points outward
%   of $X$ without adding or modifying interior stagnation points.
%   The claim then follows for this modified vector field from the Poincaré-Hopf theorem (theorem \ref{th:poincareHopf}).
% \end{proof}
% Note that this result also holds more generally if we drop the assumption that $u$ be harmonic and use relation \ref{eq:n3_MorseIneq_general}
% in the proof.
% For this we state a version of the hairy ball theorem.
% \begin{proposition}[Variant of the hairy ball theorem]
%   Let $X$ be a compact manifold with corners and $f\colon X\to \R$ a smooth Morse function without critical points. Then the
%   Euler characteristic $\chi\brk*{X}=0$ vanishes.
% \end{proposition}
% \begin{proof}
%   This follows directly from the Morse inequalities in theorem \ref{th:MorseInequalities}.
% \end{proof}
Thus for any such vector field the number of stagnation points has to be even.
We can also give a second condition on the domain topology which is similar to the hairy
ball theorem.
\begin{proposition}[Condition on the boundary topology]
  % Let $X$ and $u$ be as in corollary \ref{co:n3_conditionTypeNbrII} where we drop the requirement on $u$ to be harmonic.
  Let $X$ be a orientable manifold with smooth boundary and $u\colon X\to TX$ be a vector field without boundary stagnation points.
  % Assume additionally that a boundary component $\Gamma\subseteq\Sigma$ is a smooth manifold.
  Then the Euler characteristic $\chi\brk*{\Gamma}=0$ vanishes for each boundary component $\Gamma\subseteq\Sigma$.
\end{proposition}
\begin{proof}
  Since the projection $u_\Gamma$ onto $\Gamma$ does not vanish on $\Gamma$ by assumption it follows that the
  Poincaré-Hopf index $\ind^\text{PH}\brk*{u_\Gamma}=0$ vanishes. The claim then follows from the Morse
  index theorem.
\end{proof}

% \newpage
% \tikzset{external/export next=false}
% \tikz[remember picture,overlay] \node[opacity=0.3,inner sep=0pt] at (current page.center){\includegraphics[width=\paperwidth,height=\paperheight]{../Art/circular_001.pdf}};
% \clearpage
% \printnomenclature
