...
The final part of the proof is then given in all details in \ref[§I.5]{Milnor1963}.
We shall however sketch for the equality in equation \eqref{}.
For this define a generalisation of the Euler characteristic by
\begin{align}
  \chi\brk*{X,Y}\leftdef \sum_k\dim_\R H_k\brk*{X,Y}\,.
\end{align}
One can show that this is additive, in the sense that given suitable $X\subseteq Y\subseteq Z$
we have that
\begin{align}
  \chi\brk*{X,Z}=\chi\brk*{X,Y}+\chi\brk*{Y,Z}
\end{align}
...
\section{Relations between the domain topology and the number of stagnation points}
