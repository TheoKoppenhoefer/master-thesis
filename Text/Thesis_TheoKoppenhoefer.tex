...
% examinor
\examinor{Examinor: Evgeny Lokharu}
...
\todoin[]{
  ...
  \begin{itemize}
    ...
    \item What can be said about singular sets with no in- or outflow in 3D?
    \item define relative homol. / euler characteristic
...
Note that a harmonic function $f$ gives rise to a harmonic vector field via $u=\nabla f$.
In general a harmonic vector field $u$ is not the gradient of a harmonic function as example \ref{}
will illustrate.
...
\begin{proposition}[Harmonic vector fields on simply connected domains]\label{pr:n2:hvf:simplyConnected}
  Let $d\in\brk[c]*{2,3}$ and $\Omega\subseteq\R^d$ be open and simply connected and $u$ be a harmonic vector field. Then
  \begin{enumerate}
    \item 
...
\begin{definition}[Emergent and entrant boundary]\label{df:emergentEntrantBd}
  ...
  \begin{align}
    \Sigma^0\leftdef\Sigma^{\leq0}\cup\Sigma^{\geq0}\bigsetminus\brk*{\Sigma^+\cup\Sigma^-}\subseteq\Sigma\,.
  \end{align}
\end{definition}
...
The following are slight generalisations of definitions given in \cite[p.138f]{Shelton1980}, \cite[§5]{Morse1969} and \cite[p.282f]{Morse1970}
...
\begin{definition}[Stagnation points]\label{df:nonDegeneracy}
  ...
  If $x\in\openX$ then we call $x$ an \emph{interior stagnation point}.
...
\chapter{The Morse inequalities}
...
Let $H_k(X)$ denote the $k$-th homology space of $X$ with coefficients in the field $\K$.
...
\begin{align}
  b_k\leftdef\dim_\K H_k(X)\,.\label{eq:df_bettiNbrs}
\end{align}
...
With the Betti numbers we can define the \emph{Euler characteristic} to be
\begin{align*}
...
\begin{align}
  \sum_{k=0}^d \brk*{-1}^{k}\essind_k(f)= \chi(X)\label{eq:MorseIneq_eqality}
\end{align}
where $\chi(X)$ denotes the Euler characteristic.
\end{theorem}
\begin{proof}
  A proof for manifolds with $C^1$ boundary is given for instance in \cite[Theorem 10.2']{Morse1969}.
...
The main idea is to inspect how the topology of the manifold $X^a$ changes as we vary $a$.
...
Proofs of these classical Morse inequalities can be found in \cite{Milnor1963} and \cite{Morse1969}.
...
as to why equation \eqref{eq:MorseIneq_eqality} is reasonable.
...
In both \cite{Handron2002} one first constructs a 
...
\begin{proposition}[Morse's lemma, {\cite[Lemma 5]{Handron2002}}]
  Let $f\colon X\to\R$ be $C^{2}$ and $x$ be an essential non-degenerative
  critical point of index $k$ on the stratum $X_j$ of dimension $n$. Then there exists a
  local chart $\varphi\colon H\to U$ such that
  \begin{align*}
    f\circ\varphi^{-1}(y)=f(x)-\sum_{j=1}^ky_j^2+\sum_{j=k+1}^ny_j^2+\sum_{j=n}^dy_j\,.
  \end{align*}
\end{proposition}
\begin{proof}
  ...
  Note that the definition of an essential critical point in \ref{Handron2002} 
  fulfilling property $(3)$ also encompasses our definition of a non-degenerate
  essential critical point. Hence this statement follows from \cite[Lemma 5]{Handron2002}.
  The idea of the proof involves first applying the Morse lemma on the stratum of the critical point.
  Then this coordinate chart is extended with Taylor in a suitable manner to be a
  full coordinate chart on $X$ such that \ref{} holds.
\end{proof}
...
\begin{lemma}[Interval with critical value, {\cite[Theorem 8]{Handron2002}} and {\cite[Theorem 2.2]{Agrach1991}}]
  If the interval $[a,b]$ contains one critical value $c\in\R$ with non-degenerate essential critical point
  $x$ of index $k$.
  then $X^b$ is homology equivalent to $X^a$ by attaching the $k$-cell
  $D^{k}$.
\end{lemma}
\begin{proof}
  ...
  It then proceeds in three steps.
  First one defines a region $H$ as in figure \ref{}.
  Then one shows that $X^a$ with $H$ attached is homotopic to $X^b$.
  Finally one shows that $X^a$ with a $k$-cell attached is homotopic to
  $X^a$ with $H$ attached.
\end{proof}


We require a result from algebraic topology:
...
\end{lemma}
\begin{proof}
  See \cite[Theorem 2.20]{Hatcher2002}.
\end{proof}

The final part of the proof is then given in all details in \cite[§I.5]{Milnor1963}.
Since we only use the equality in equation \eqref{eq:MorseIneq_eqality} we shall outline its proof.
For this define a generalisation of the Euler characteristic by
\begin{align}
  \chi\brk*{X,Y}\leftdef \sum_k\brk*{-1}^k\dim_\K H_k\brk*{X,Y}\,.
\end{align}
One can show that this is additive, in the sense that given suitable $X\supseteq Y\supseteq Z$
we have that
\begin{align}
  \chi\brk*{X,Z}=\chi\brk*{X,Y}+\chi\brk*{Y,Z}\label{eq:MorseIneq_additivChi}
\end{align}
Let $a_0<\dots<a_L$ be such that $[a_l,a_{l+1}]$ contains precisely one critical value with
index $k_l$ and such that $X=X^{a_L}$
It now follows from excision that
\begin{align}
  H_k\brk*{X^{a_{l+1}},X^{a_{l}}}
  &=H_k\brk*{X^{a_{l}}\cup D^{k_l},X^{a_{k}}} \\
  &=H_k\brk*{D^{k_l},S^{k_l}} \\
  &=\begin{cases}
    \K &\text{ if }k=k_l,\\
    0 &\text{ else}
  \end{cases}\,.
\end{align}
Hence we obtain by inductively applying \eqref{eq:MorseIneq_additivChi}
and then equation 
\begin{align*}
  \chi\brk*{X}=\sum_k \chi\brk*{X^{x_{k+1}},X^k}
...
%   \td{Write some proof.}
% \end{proof}
As a final result in this chapter we inspect the Morse inequalities in the special case
that $f$ is harmonic.
If we assume that $f$ is harmonic the maximum principle implies that
$M_0=0=M_d$.
If we additionally assume that we have dimensions $d=2$ we obtain from \ref{} a result from \cite[Corollary 10.1]{Morse1969}.
\begin{corollary}[Morse inequalities for harmonic $f$ in $\R^2$, {\cite[Corollary 10.1]{Morse1969}}]
  ...
  \end{align*}
\end{corollary}
In dimensions $d=3$ we obtain from \ref{} a result from \cite[Corollary 10.2]{Morse1969}.
...
\end{proposition}
\begin{proof}
  See \cite[Theorem 2.1]{Alessandrini1992} and \cite[Theorem 2.2]{Alessandrini1992} where we set $\underline{\alpha}=n$ to be the outer unit normal and $D=\chi\brk*{X}$
  to be the Euler characteristic.
\end{proof}
If we set $X$ to be homeomorphic to the ball, that is $\chi\brk*{X}=1$, and require the entrant
boundary to be simply connected, that is $J^\pm=J^{\geq0}=1$, we then obtain from proposition \ref{pr:alessandrini_inflowOutflow} that $M\leq0$
and $f$ has no non-degenerate critical point.
...
\chapter{Simply connected entrant boundaries in $\R^3$}
...
\begin{example}[A harmonic function with interior critical point and simply connected entrant boundary]\label{ex:n3_polynomial_defn}
...
% In example \ref{} we show that $\Sigma^+$ is separated from $\Sigma^-$ by the curve
% $C=V\brk*{}$. In the following we will construct an example in which the entrant and
% emergent boundaries are simply connected and we have an interior stagnation point.

% \begin{example}
%   Let $f$ and $X$ be as in example \ref{}. Define for $\rho>0$ the cutoff function $\theta_\rho\colon\Sigma\to\R$ by the
%   condition that $\theta_\rho=1$ outside of $\tubular_{2\rho}\brk*{C}$ and 
%   support contained in $\supp\theta_\rho\subseteq\Sigma\setminus\tubular_{\rho}\brk*{C}$.
%   Consider now the solution $f_\rho$ to the system
%   \begin{align*}
%     \Delta f_\rho&=0 &&\text{ on }\Omega, \\
%     \nabla f_\rho\cdot n&=\theta_\rho\brk*{\nabla f\cdot n} &&\text{ on }\Sigma, \\
%     \brk[a]*{f_\rho}_\Omega&=\brk[a]*{f}_\Omega &&
%   \end{align*}
...
%   Thus it remains to be shown that there exists $\rho>0$ such that $f_\rho$ has an interior
%   critical point.
%   We claim that
%   \begin{align*}
%     \norm*{f_\rho-f}_{L^\infty\brk*{B_2}}\longrightarrow0\text{ as }\rho\to0\,.
%   \end{align*}
...
%     &\stackrel{\text{divergence theorem}}{=}\brk*{\int_\Sigma\brk*{\theta_\rho-1}\brk*{\nabla f\cdot n}\dif x}^{1/2}\longrightarrow0
%   \end{align*}
