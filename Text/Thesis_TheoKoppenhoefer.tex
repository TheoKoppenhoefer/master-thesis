...
\newcommand{\annotateAbove}[5][]{
  ...
  \begin{tikzpicture}[remember picture, overlay, node distance = 0.5cm]
    \node[,text width=5cm] (annotateAbove_\arabic{annotateAboveCtr}_descr) [above right= #4cm and #5cm of annotateAbove_\arabic{annotateAboveCtr}]{#2};
...
\begin{definition}[Stratified space, \cite{Handron2002}]\label{df:stratified_space}
  ...
  If additionally $n(k)=0$ or $n(k)=n(j)+1$ we will
...
\nomenclature{$\precsim$}{We write $X_j\precsim X_k$ if $X_j\prec X_k$ and an additional condition holds. See definition \ref{df:stratified_space} for details.}
...
\begin{proof}
  ...
  Conversely assume that $x$ is stagnation point for some stratum $X_{j+1}$. Then we have that
  ...
  and thus $x$ is an irregular stagnation point.
\end{proof}
% \begin{proof}
